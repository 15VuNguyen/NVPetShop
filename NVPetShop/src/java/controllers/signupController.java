        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            for (Account a : list) {
                if (a.getEmail().equalsIgnoreCase(email)) {
                    session.setAttribute("tmp1","Email is existed!");
                    request.getRequestDispatcher("mainController?action=home").forward(request, response);
                }
            }
            int result = d.insertAccount(Username, email, Password);
            if (result > 0) {
                Account acc = d.getAccount(email, Password);
                session.setAttribute("logined", acc);
                request.getRequestDispatcher("mainController?action=home").forward(request, response);
            } else {
                request.getRequestDispatcher("mainController?aciotn=error").forward(request, response);
            }
        }
    }
